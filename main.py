...
class CanvasLayout(FloatLayout):
    ...
    def on_touch_up(self, touch):
        ...
        if SettingsLayout.state == None:
            ...
            mesh_object.center = [points[0][0],points[0][1]]
            ...
            square_point = []
            square_point.extend([points[1][0] - 20, points[1][1] - 20, 0, 0])
            square_point.extend([points[1][0] - 20, points[1][1] + 20, 0, 1])
            square_point.extend([points[1][0] + 20, points[1][1] + 20, 1, 1])
            square_point.extend([points[1][0] + 20, points[1][1] - 20, 1, 0])
            ...
            second_object = MeshSquare(name=self.nextName(), color=second_color, mass=1,pendulum=True)

            first_object.vertices = circle_point_static
            second_object.vertices = square_point
            first_object.indices = range(len(circle_point_static) // 4)
            second_object.indices = range(len(square_point) // 4)
            ...
            mesh_object.setMinMax(min(points[0][0]-2, points[1][0]-2), max(points[0][0]+2, points[1][0]+2),
                                  min(points[0][1]-2, points[1][1]-2), max(points[0][1]+2, points[1][1]+2))
            points_ext = mesh_object.extreme_points()
